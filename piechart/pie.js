var nb_sliders = 3; // nb of range sliders
...
// set the # of sliders to have and initiate the sliders and pie chart

d3.select('#dualslider').call(d3.slider().axis(true).value( [ 10, 25 ] ).on("slide", function(evt, value) {
...
function init() {
  ...
  for (i = 0; i < nb_sliders; i++) {
    console.log("nb_sliders ", nb_sliders)

    var tr = d3.select('#rangebox tbody').selectAll("tr").append('tr');
    tr.append('td')
      .attr('class', 'edit')
  ...
  d3.selectAll('.range').on('input', function() {
    console.log("Inside the slider event")
